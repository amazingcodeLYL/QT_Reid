...
def non_max_suppression(predsiction, conf_thres=0.5, nms_thres=0.5):
    """
    剔除物体置信度得分object confidence score低于'conf_thres'阈值的检测框
    再利用NMS进一步过滤筛选检测框
    :param predsiction: 处理后的YOLO预测，相对于原图 torch.Size([1, 8190, 85])
    ...
    output = [None] * len(predsiction)
    for image_i, preds in enumerate(predsiction):
        ...
        # class_preds:每个预测框概率最高的那个类别       torch.Size([8190])
        class_conf, class_preds = preds[:, 5:].max(1)
        ...
        # Select only suitable predsictions
        # 1.preds[:, 4] > conf_thres: 保留置信度阈值大于阈值的预测框
        # 2.(preds[:, 2:4] > min_wh).all(1) > min_wh: 保留w,h > min_wh = 2的预测框，太小的忽略不计
        # 3.torch.isfinite(preds).all(1): 保留预测值都正常的
        # torch.isfinite(preds) 返回一个新的张量，其布尔元素表示每个元素是否为+/-INF,是INF NAN则返回0
        i = (pred_s[:, 4] > conf_thres) & (pred_s[:, 2:4] > min_wh).all(1) & torch.isfinite(pred_s).all(1)
        pred_s = pred_s[i] # 经过NMS，只剩下29个预测框 torch.Size([29, 85])

        # If none are remaining => process next image
        if len(pred_s) == 0:
            continue

        # Select pred_sicted classes
        # 进行NMS时，使用的是类别概率乘以confidence
        # 这里选择类别只用类别概率
        class_conf = class_conf[i]
        class_preds = class_preds[i].unsqueeze(1).float()

        # Box (center x, center y, width, height) to (x1, y1, x2, y2)
        pred_s[:, :4] = xywh2xyxy(pred_s[:, :4]) # torch.Size([29, 85])

        # Detections ordered as (x1y1x2y2, obj_conf, class_conf, class_preds)
        pred_s = torch.cat((pred_s[:, :5], class_conf.unsqueeze(1), class_preds), 1) # 通过这样NMS后，只剩下29个框torch.Size([29, 7])

        # Get detections sorted by decreasing confidence scores
        # 对于剩下的预测框安装置信度(类别概率乘以confidence)得分进行排序
        pred_s = pred_s[(-pred_s[:, 4]).argsort()]
        ...
        for c in pred_s[:, -1].unique():   # 没有80类一个个遍历，更高效
            dc = pred_s[pred_s[:, -1] == c]  # select class c torch.Size([21, 7]) 代表这个类别有21个预测框
            n = len(dc) # 当前类别有len(dc)=21个预测框
            if n == 1:
                det_max.append(dc)  # No NMS required if only 1 predsiction
                continue
            elif n > 100:
